...
st.title("📡 THE RADAR")
st.caption("FRENERGY STRATEGIC PROCUREMENT INTELLIGENCE (DEADLINE FOCUS)")
st.divider()
...
if st.sidebar.button("🔍 전략 수색 개시", type="primary"):
    ...
    # 🎯 날짜 파라미터 재설정 (마감일 기준)
    s_date = (now - timedelta(days=7)).strftime("%Y%m%d") # 공고일 검색용(과거 7일)
    today_str = now.strftime("%Y%m%d")
    
    # 부장님 요청: 내일부터 7일간 마감되는 건들 타격
    tomorrow_dt = (now + timedelta(days=1)).strftime("%Y%m%d")
    deadline_limit_dt = (now + timedelta(days=7)).strftime("%Y%m%d")
    ...
    try:
        ...
        for i, kw in enumerate(KEYWORDS):
            ...
            try:
                ...
                for it in items:
                    clos_dt = re.sub(r'[^0-9]', '', it.get('bidClseDt', ''))[:8]
                    # 🎯 마감일 필터링 (내일~7일이내)
                    if tomorrow_dt <= clos_dt <= deadline_limit_dt:
                        final_list.append({
        ...
        try:
            ...
            # LH는 공고일 기준으로 먼저 긁어온 후 마감일로 필터링
            ...
            for item in root.findall('.//item'):
                ...
                open_dt = re.sub(r'[^0-9]', '', item.findtext('openDtm', ''))[:8]
                if any(kw in bid_nm for kw in KEYWORDS) and (tomorrow_dt <= open_dt <= deadline_limit_dt):
                    final_list.append({
                        '출처':'LH', '번호':item.findtext('bidNum'), '공고명':bid_nm, 
                        '수요기관':'LH', '예산':int(pd.to_numeric(item.findtext('fdmtlAmt') or 0, errors='coerce') or 0), 
                        '지역':'전국', '마감일':format_date_clean(item.findtext('openDtm')), 
                        'URL':f"https://ebid.lh.or.kr/ebid.et.tp.cmd.BidsrvcsDetailListCmd.dev?bidNum={item.findtext('bidNum')}&bidDegree=00"
                    })
        except: pass

        # --- 3. 국방부 (D2B 마감일 정밀 타격) ---
        status_st.info("📡 [PHASE 3] D2B 수색 중 (마감일 기준)...")
        # 수의계약: 부장님 요청하신 마감일 범위로 직접 검색
        try:
            p_priv = {'serviceKey': SERVICE_KEY, 'numOfRows': '500', '_type': 'json', 
                      'prqudoPresentnClosDateBegin': tomorrow_dt, 
                      'prqudoPresentnClosDateEnd': deadline_limit_dt}
        ...
        try:
            ...
            for item in root_kg.findall('.//item'):
                ...
                end_dt = re.sub(r'[^0-9]', '', item.findtext('END_DT', ''))[:8]
                if any(kw in title for kw in KOGAS_KEYWORDS) and (tomorrow_dt <= end_dt <= deadline_limit_dt):
                    final_list.append({
                        '출처': 'KOGAS', '번호': item.findtext('NOTICE_CODE') or '-', '공고명': title, 
                        '수요기관': '가스공사', '예산': 0, '지역': '전국', 
                        '마감일': format_date_clean(item.findtext('END_DT')), 
                        'URL': KOGAS_DIRECT_URL
                    })
        except: pass
        ...
        if final_list:
            ...
            st.success(f"✅ 내일부터 7일간 마감되는 공고 총 {len(df)}건을 발견했습니다.")
            st.dataframe(df.style.format({'예산': '{:,}원'}), use_container_width=True)
            
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                df.to_excel(writer, index=False, sheet_name='RADAR_REPORT')
            st.download_button(label="📥 전략 리포트 다운로드", data=output.getvalue(), file_name=f"RADAR_DEADLINE_{today_str}.xlsx")
        else:
            st.warning("⚠️ 해당 기간(내일~7일이내)에 마감되는 공고가 없습니다.")
