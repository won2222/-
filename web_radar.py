...
if st.sidebar.button("📡 전 구역 정밀 수색", type="primary"):
    final_list = []
    now = datetime.now()
    # 수집 기간: 과거 7일부터 미래 3일까지 넉넉하게 잡음
    s_date = (now - timedelta(days=7)).strftime("%Y%m%d")
    e_date = (now + timedelta(days=3)).strftime("%Y%m%d")
    ...
    try:
        ...
        for i, kw in enumerate(KEYWORDS):
            ...
            try:
                p = {'serviceKey': SERVICE_KEY, 'numOfRows': '100', 'type': 'json', 'inqryDiv': '1', 'inqryBgnDt': s_date+'0000', 'inqryEndDt': e_date+'2359', 'bidNtceNm': kw}
        ...
        try:
            ...
            # LH는 검색 기간을 넉넉히 설정해야 잡힘
            p_lh = {'serviceKey': SERVICE_KEY, 'numOfRows': '500', 'tndrbidRegDtStart': (now - timedelta(days=15)).strftime("%Y%m%d"), 'tndrbidRegDtEnd': e_date}
                ...
                for it in items:
                    ...
                    if any(kw in bid_nm for kw in KEYWORDS):
                        ...
                        p_det = {
                            ...
                            'dcsNo': it.get('dcsNo'),
                            'iemNo': it.get('iemNo'), # 공개수의용
                            'ntatPlanDate': it.get('ntatPlanDate'), # 공개수의용
                            '_type': 'json'
                        }
                        ...
                        try:
                            ...
                            if det:
                                ...
                                area = det.get('areaLmttList') or "국방부상세"
                        except:
                            budget = it.get('asignBdgtAmt') or it.get('budgetAmount') or 0

                        final_list.append({
                            '출처':'2.국방부', '번호':it.get('pblancNo') or it.get('dcsNo'), 
                            '공고명':bid_nm, '수요기관':it.get('ornt') or "국방부", 
                            '예산':int(pd.to_numeric(budget, errors='coerce') or 0), 
                            '지역':area, '마감일':format_date_clean(it.get(config['clos'])), 
        ...
        if final_list:
            ...
            status.success(f"✅ 작전 완료! 3사 통합 {len(df)}건을 찾았습니다.")
