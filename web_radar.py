...
if st.sidebar.button("📡 전 구역 정밀 수색", type="primary"):
    ...
    try:
        # --- 1. 나라장터 (G2B) ---
        status.info("📡 [1단계] 나라장터 수집 및 면허/지역 필터링 중...")
        ...
        try:
            ...
            p_lh = {'serviceKey': SERVICE_KEY, 'numOfRows': '1000', 'pageNo': '1', 'tndrbidRegDtStart': (now - timedelta(days=10)).strftime("%Y%m%d"), 'tndrbidRegDtEnd': today_str}
            ...
            if res_lh.status_code == 200:
                ...
                for item in root.findall('.//item'):
                    ...
                    if any(kw in bid_nm for kw in KEYWORDS):
                        ...
                        final_list.append({'출처':'3.LH', '번호':b_no, '공고명':bid_nm.strip(), '수요기관':'한국토지주택공사', '예산':int(pd.to_numeric(item.findtext('fdmtlAmt') or 0, errors='coerce') or 0), '지역':'전국/상세참조', '마감일':format_date_clean(item.findtext('openDtm')), 'URL':f"https://ebid.lh.or.kr/ebid.et.tp.cmd.BidsrvcsDetailListCmd.dev?bidNum={b_no}&bidDegree=00"})
        except: pass

        # --- 3. 국방부 (부장님 정밀 로직) ---
        status.info("📡 [3단계] 국방부(D2B) 예산 정밀 추적 중...")
        d2b_configs = [
            {'op': 'getDmstcCmpetBidPblancList', 'det': 'getDmstcCmpetBidPblancDetail', 'clos': 'biddocPresentnClosDt'},
            {'op': 'getDmstcOthbcVltrnNtatPlanList', 'det': 'getDmstcOthbcVltrnNtatPlanDetail', 'clos': 'prqudoPresentnClosDt'}
        ]
        for config in d2b_configs:
            try:
                ...
                for it in items:
                    ...
                    if any(kw in bid_nm for kw in KEYWORDS):
                        p_det = {'serviceKey': SERVICE_KEY, 'pblancNo': it.get('pblancNo'), 'pblancOdr': it.get('pblancOdr'), 'demandYear': it.get('demandYear'), 'orntCode': it.get('orntCode'), 'dcsNo': it.get('dcsNo'), 'iemNo': it.get('iemNo'), 'ntatPlanDate': it.get('ntatPlanDate'), '_type': 'json'}
                        budget = 0
                        try:
                            ...
                            if det: budget = det.get('budgetAmount') or it.get('asignBdgtAmt') or 0
                        except: budget = it.get('asignBdgtAmt') or 0
                        
                        final_list.append({'출처':'2.국방부', '번호':it.get('pblancNo') or it.get('dcsNo'), '공고명':bid_nm, '수요기관':it.get('ornt') or "국방부", '예산':int(pd.to_numeric(budget, errors='coerce') or 0), '지역':'국방부상세', '마감일':format_date_clean(it.get(config['clos'])), 'URL':'https://www.d2b.go.kr'})
            except: continue

        if final_list:
            df = pd.DataFrame(final_list).drop_duplicates(subset=['번호']).sort_values(by=['출처', '마감일'])
            df['출처'] = df['출처'].str.replace(r'^[0-9]\.', '', regex=True)
            status.success(f"✅ 작전 완료! 총 {len(df)}건을 확보했습니다.")
            ...
            status.warning("⚠️ 현재 조건에 맞는 공고가 없습니다.")
