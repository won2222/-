...
if st.sidebar.button("🔍 전략 수색 개시", type="primary"):
    ...
    target_end_day = (now + timedelta(days=30)).strftime("%Y%m%d")
    ...
    try:
            ...
            try:
                ...
                for item in root.findall('.//item'):
                    ...
                    if any(kw in bid_nm for kw in KEYWORDS):
                        ...
                        final_list.append({'출처':f'LH({"공사" if job_gb=="1" else "용역"})', '번호':b_no, '공고명':bid_nm, '수요기관':'LH', '예산':int(pd.to_numeric(item.findtext('fdmtlAmt') or 0, errors='coerce') or 0), '지역':'전국', '마감일':format_date_clean(item.findtext('openDtm')), 'URL':f"https://ebid.lh.or.kr/ebid.et.tp.cmd.BidsrvcsDetailListCmd.dev?bidNum={b_no}&bidDegree=00"})
            except: pass

        # --- 3. 국방부 (v161.0 정밀 로직) ---
        status_st.info("📡 [PHASE 3] D2B 수색 중...")
        d2b_api = [{'t': '일반', 'l': 'getDmstcCmpetBidPblancList', 'd': 'getDmstcCmpetBidPblancDetail', 'c': 'biddocPresentnClosDt'}, {'t': '수의', 'l': 'getDmstcOthbcVltrnNtatPlanList', 'd': 'getDmstcOthbcVltrnNtatPlanDetail', 'c': 'prqudoPresentnClosDt'}]
        for cfg in d2b_api:
            try:
                p_d = {'serviceKey': SERVICE_KEY, 'numOfRows': '500', '_type': 'json'}
                if cfg['t'] == '수의': p_d.update({'prqudoPresentnClosDateBegin': s_date, 'prqudoPresentnClosDateEnd': (now + timedelta(days=30)).strftime("%Y%m%d")})
                ...
                for it in items_d:
                    ...
                    clos_dt_full = it.get(cfg['c'])
                    if any(kw in bid_nm for kw in KEYWORDS) and str(clos_dt_full)[:8] >= today_str:
                        ...
                        if any(t in area for t in MUST_PASS_AREAS):
                            final_list.append({'출처': f"D2B({cfg['t']})", '번호': p_no, '공고명': bid_nm, '수요기관': it.get('ornt'), '예산': int(pd.to_numeric(budget, errors='coerce') or 0), '지역': area, '마감일': format_date_clean(clos_dt_full), 'URL': 'https://www.d2b.go.kr'})
            except: continue
        ...
        for m_kw in [last_month, search_month]:
            for kw in KWATER_KEYWORDS:
                try:
                    res_k = requests.get("http://apis.data.go.kr/B500001/ebid/tndr3/servcList", params={'serviceKey': SERVICE_KEY, 'searchDt': m_kw, 'bidNm': kw, 'numOfRows': '500', '_type': 'json'}, timeout=10).json()
                    ...
                    for kit in k_items:
                        ...
                        if any(k in title for k in KWATER_KEYWORDS):
                            final_list.append({'출처': 'K-water', '번호': raw_no, '공고명': title, '수요기관': '한국수자원공사', '예산': 0, '지역': '전국', '마감일': format_date_clean(kit.get('tndrPblancEnddt')), 'URL': f"{KWATER_DETAIL_BASE}{raw_no}"})
                except: continue
