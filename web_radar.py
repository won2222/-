...
st.title("📡 THE RADAR")
st.caption("FRENERGY STRATEGIC PROCUREMENT - LH PRECISION LOADED")
st.divider()
...
if st.sidebar.button("🔍 전 기관 통합 수색 개시", type="primary"):
    ...
    try:
        # --- 🎯 1. LH (최우선 순위 배치 + Timeout 연장) ---
        status_st.info(f"📡 [1/5] LH 정밀 수색 중... ({lh_s_str} ~ {lh_e_str})")
        try:
            ...
            # LH 서버의 느린 응답을 위해 timeout을 25초로 대폭 연장
            ...
            st.sidebar.warning(f"LH 연결 지연: {e}") # 에러나도 멈추지 않고 계속 진행

        prog.progress(20)

        # --- 2. 나라장터 (G2B) ---
        status_st.info("📡 [2/5] 나라장터 수색 및 필터링 중...")
        ...
        # --- 3. 국방부 (v161.0 정밀 로직) ---
        status_st.info("📡 [3/5] 국방부 수의/일반 수색 중...")
        ...
        # --- 4. 수자원 & 5. 가스공사 ---
        status_st.info("📡 [4,5/5] 수자원/가스공사 수색 중...")
        # (생략 없이 기존 로직 그대로 수행)
        for kw in KWATER_KEYWORDS:
            try:
                ...
                for kit in ([res_k['response']['body']['items']['item']] if isinstance(res_k['response']['body']['items']['item'], dict) else res_k['response']['body']['items']['item']):
        ...
        if final_list:
            ...
            st.download_button(label="📥 통합 리포트(Excel) 저장", data=output.getvalue(), file_name=f"RADAR_{today_str}.xlsx")
        else:
            st.warning("⚠️ 해당 기간에 맞는 공고가 없습니다.")
