...
if st.sidebar.button("🔍 전 기관 통합 수색 개시", type="primary"):
    ...
    # [파일 참고] 날짜 파라미터 복구
    s_date_api = (now - timedelta(days=7)).strftime("%Y%m%d") # 나라장터, LH (7일치)
    today_str = now.strftime("%Y%m%d")
    search_month = now.strftime('%Y%m') # 수자원공사용
    kogas_start = (now - timedelta(days=180)).strftime("%Y%m%d") # 가스공사 6개월 로직
    ...
    try:
        ...
        try:
            ...
            for it in it_priv:
                ...
                if any(kw in bid_nm for kw in KEYWORDS):
                    final_list.append({'출처':'D2B', '번호':it.get('dcsNo'), '공고명':bid_nm, '수요기관':it.get('ornt'), '예산':int(pd.to_numeric(it.get('budgetAmount', 0), errors='coerce') or 0), '지역':'국방부', '마감일':format_date_clean(it.get('prqudoPresentnClosDt')), 'URL':'https://www.d2b.go.kr'})
        except: pass

        # --- 4. 수자원공사 (파일: 수자원공사 완성.py 로직) ---
        ...
        try:
            ...
            for item in root_kg.findall('.//item'):
                ...
                if any(kw in title for kw in KOGAS_KEYWORDS):
                    final_list.append({'출처': 'KOGAS', '번호': item.findtext('NOTICE_CODE'), '공고명': title, '수요기관': '한국가스공사', '예산': 0, '지역': '전국', '마감일': format_date_clean(item.findtext('END_DT')), 'URL': "https://bid.kogas.or.kr:9443/supplier/index.jsp"})
        except: pass
        ...
        if final_list:
            ...
            st.write("")
            st.success(f"✅ 총 {len(df)}건 확보! (공고일/마감일 기준 통합 작전 성공)")
            ...
            st.download_button(label="📥 통합 전략 리포트 다운로드", data=output.getvalue(), file_name=f"RADAR_CLASSIC_{today_str}.xlsx")
        else:
