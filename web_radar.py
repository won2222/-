...
# --- [2] 웹 화면 구성 ---
st.set_page_config(page_title="3사 통합 레이더 v292", layout="wide")
st.title("📡 THE RADAR (전략 수색 시스템)")

# --- [3] 사이드바: 수색 범위 설정 (부장님 요청 사항) ---
st.sidebar.header("🕹️ 수색 범위 설정")
search_days = st.sidebar.slider("수색 과거 기간 (공고 등록일 기준)", 1, 14, 7)
deadline_days = st.sidebar.slider("수색 미래 기간 (국방부 마감일 기준)", 1, 14, 3)
st.sidebar.divider()
...
if st.sidebar.button("🔍 전 구역 정밀 수색 시작", type="primary"):
    ...
    # 나라장터/LH 검색 기준 (설정된 과거 기간 적용)
    s_date = (now - timedelta(days=search_days)).strftime("%Y%m%d")
    today_str = now.strftime("%Y%m%d")
    
    # 국방부 마감일 필터 (설정된 미래 기간 적용)
    tomorrow_str = (now + timedelta(days=1)).strftime("%Y%m%d")
    target_end_day = (now + timedelta(days=deadline_days)).strftime("%Y%m%d")
    ...
    try:
        # --- 1. 나라장터 (G2B) ---
        status_st.info(f"📡 [1단계] 나라장터 수집 ({search_days}일치: {s_date} ~ {today_str})")
        ...
        # --- 2. LH ---
        status_st.info(f"📡 [2단계] LH 시설공사 수집 ({search_days}일치: {s_date} ~ {today_str})")
        try:
        ...
        # --- 3. 국방부 (v161.0 정밀 로직) ---
        status_st.info(f"📡 [3단계] 방위사업청 수색 (마감 {deadline_days}일 내: {tomorrow_str} ~ {target_end_day})")
        api_configs = [
        ...
        if final_list:
            ...
            st.download_button(label="📥 통합 리포트(Excel) 다운로드", data=output.getvalue(), file_name=f"RADAR_{today_str}_{search_days}d.xlsx")
        else: status_st.warning("⚠️ 선택하신 수색 범위 내에 조건에 맞는 공고가 없습니다.")
