    ...
    try:
        ...
        for op in ['getDmstcCmpetBidPblancList', 'getDmstcOthbcVltrnNtatPlanList']:
            try:
                ...
                for it in items:
                    ...
                    if any(kw in bid_nm for kw in KEYWORDS):
                        # 🎯 1차 예산 확보 (기본 리스트에서)
                        raw_budget = it.get('asignBdgtAmt') or it.get('dcsBdgtAmt') or it.get('budgetAmount') or 0
                        
                        # 🎯 2차 예산 확보 (상세 조회를 통해 정밀하게)
                        try:
                            # dcsNo가 있으면 dcsNo로, 없으면 pblancNo로 조회
                            target_no = it.get('dcsNo') or it.get('pblancNo')
                            url_det = f"http://openapi.d2b.go.kr/openapi/service/BidPblancInfoService/{op.replace('List', 'Detail')}"
                            p_det = {'serviceKey': SERVICE_KEY, 'pblancNo': target_no, 'dcsNo': target_no, '_type': 'json'}
                            
                            det_res = requests.get(url_det, params=p_det, headers=HEADERS, timeout=3).json()
                            det = det_res.get('response', {}).get('body', {}).get('item', {})
                            
                            # 상세 항목에서 예산 필드 싹 다 뒤지기
                            if det:
                                raw_budget = det.get('budgetAmount') or det.get('dcsBdgtAmt') or det.get('asignBdgtAmt') or raw_budget
                        except: pass
                        ...
                        final_list.append({
                            ...
                            '예산':int(pd.to_numeric(raw_budget, errors='coerce') or 0), 
                            '지역':'국방부상세', 
        ...
        if final_list:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                worksheet.autofilter(0, 0, len(df), len(df.columns) - 1) # 엑셀 필터 추가
                for i, _ in enumerate(df.columns): worksheet.set_column(i, i, 18) # 간격 조정
